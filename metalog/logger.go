...
// DefaultLogger is the global metalog.
...
// Entry represents a metalog entry. It is instanced by one of the level method of Logger and finalized by the Msg or Msgf method.
type Entry struct {
	buf   []byte
	Level Level
	w     Writer
...
// ordinary functions as metalog writers. If f is a function
...
type Logger struct {
	...
	// Strongly recommended to leave TimeFormat empty for optimal built-in metalog formatting performance.
	...
	// Context specifies an optional context of metalog.
	Context Context
...
// Printf sends a metalog entry without extra field. Arguments are handled in the manner of fmt.Printf.
...
// SetLevel changes metalog default level.
...
// AnErr adds the field key with serialized err to the metalog context.
...
// RawJSON adds already encoded JSON to the metalog line under key.
...
// RawJSONStr adds already encoded JSON String to the metalog line under key.
...
// Enabled return false if the entry is going to be filtered out by metalog level.
