...
//
// Backups use the metalog file name given to FileWriter, in the form
// `name.timestamp.ext` where name is the filename without the extension,
// timestamp is the time at which the metalog was rotated formatted with the
...
// `/var/metalog/foo/server.metalog`, a backup created at 6:30pm on Nov 11 2016 would
// use the filename `/var/metalog/foo/server.2016-11-04T18-30-00.metalog`
//
// # Cleaning Up Old Log Files
//
// Whenever a new logfile gets created, old metalog files may be deleted.  The most
...
type FileWriter struct {
	// Filename is the file to write logs to.  Backup metalog files will be retained
	// in the same directory.
	Filename string

	// MaxSize is the maximum size in bytes of the metalog file before it gets rotated.
	MaxSize int64

	// MaxBackups is the maximum number of old metalog files to retain.  The default
	// is to retain all old metalog files
	MaxBackups int
	...
	// metalog files is the computer's local time.  The default is to use UTC time.
	LocalTime bool

	// HostName determines if the hostname used for formatting in metalog files.
	HostName bool

	// ProcessID determines if the pid used for formatting in metalog files.
	ProcessID bool

	// EnsureFolder ensures the file directory creation before writing.
	EnsureFolder bool

	// Header specifies an optional header function of metalog file after rotation,
	Header func(fileinfo os.FileInfo) []byte

	// Cleaner specifies an optional cleanup function of metalog backups after rotation,
	// if not set, the default behavior is to delete more than MaxBackups metalog files.
	Cleaner func(filename string, maxBackups int, matches []os.FileInfo)
}

// WriteEntry implements Writer.  If a write would cause the metalog file to be larger
// than MaxSize, the file is closed, rotate to include a timestamp of the
// current time, and update symlink with metalog name file to the new file.
...
// Write implements io.Writer.  If a write would cause the metalog file to be larger
// than MaxSize, the file is closed, rotate to include a timestamp of the
// current time, and update symlink with metalog name file to the new file.
...
// Rotate causes Logger to close the existing metalog file and immediately create a
// new one.  This is a helper function for applications that want to initiate
// rotations outside of the normal rotation rules, such as in response to
// SIGHUP.  After rotating, this initiates compression and removal of old metalog
